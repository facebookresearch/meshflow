...
from meshflow.utils.mesh import GEOMETRY_EXTS, Mesh
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "override vertex budget at test time; only effective when "
            "denoiser_model.use_proj_cond_on_temb is enabled in config "
            "(common values: 1024 / 4096)"
        ),
    )
    parser.add_argument(
        "--num_verts_max",
    ...
    pipeline = MeshFlowPipeline.from_pretrained(
        ...
        num_verts_max=args.num_verts_max,
    )
    ...
    print(
        f"  num_verts={pipeline.num_verts} num_verts_max={pipeline.num_verts_max} "
    ...
    if denoiser.use_proj_cond_on_temb:
        proj = pipeline.get_proj_cond_on_temb(guidance_scale=guidance_scale)
        print(f"  proj_cond_on_temb={proj.squeeze().item():.6f} (num_verts / num_verts_max)")
    ...
    for mesh_path in meshes:
        ...
        surface_pc = pipeline.sample_surface_points(str(mesh_path))
        ...
        out_mesh.to_trimesh().export(str(out_root / "generated_meshes" / f"{stem}.glb"))
        print(f"Saved {stem}")
