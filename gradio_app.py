...
from meshflow.utils.mesh import GEOMETRY_EXTS, MESH_EXTS, POINT_CLOUD_EXTS

NUM_VERTS_MIN = 1024
NUM_VERTS_MAX = 8192
NUM_VERTS_STEP = 256
TEST_TIME_SCALING_NOTE = (
    "Test-time scaling via `num_verts` is available only when the loaded model config "
    "sets `denoiser_model.use_proj_cond_on_temb: true`. "
    "Adjust `num_verts` (1024–8192) and `num_verts_max` (training vertex budget) accordingly."
)
...
def read_config_num_verts_max(model_path: str) -> int:
    cfg = OmegaConf.load(Path(model_path) / "config.yaml")
    scfg = cfg.system
    input_size = int(scfg.denoiser_model.input_size)
    return int(scfg.get("num_verts_max", input_size))
...
def load_pipeline(
    ...
    num_verts_max: Optional[int] = None,
) -> MeshFlowPipeline:
...
def resolve_default_num_verts(args: argparse.Namespace) -> int:
    ...
    return clamp_num_verts(read_config_num_verts(args.model_path))


def resolve_default_num_verts_max(args: argparse.Namespace) -> int:
    if args.num_verts_max is not None:
        return int(args.num_verts_max)
    return read_config_num_verts_max(args.model_path)
...
def run_meshflow(
    ...
    if getattr(runtime_args, "supports_num_verts_scaling", False):
        num_verts = clamp_num_verts(num_verts)
        num_verts_max = int(num_verts_max)
        if num_verts_max <= 0:
            raise ValueError(f"num_verts_max must be positive, got {num_verts_max}")
        ...
        if pipeline_state.get("num_verts") != num_verts:
            ...
            if pipeline.num_verts_max != num_verts_max:
                pipeline.num_verts_max = num_verts_max
                pipeline_state["num_verts_max"] = num_verts_max
    else:
        pipeline = pipeline_state["pipeline"]
    ...
    pipeline_state = gr.State(
        {
            ...
            "num_verts_max": default_num_verts_max,
        }
    )
        ...
        with gr.Row(equal_height=False):
            with gr.Column(scale=4, elem_classes="mf-input-column"):
                ...
                with gr.Accordion("Advanced options", open=False):
                    ...
                    num_verts_max = gr.Number(
                        value=default_num_verts_max,
                        precision=0,
                        label="num_verts_max",
                        visible=supports_num_verts_scaling,
                    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "initial num_verts for test-time scaling; only effective when "
            "denoiser_model.use_proj_cond_on_temb is enabled in config "
            f"({NUM_VERTS_MIN}-{NUM_VERTS_MAX}; default: from config.yaml)"
        ),
    )
    parser.add_argument(
        "--num_verts_max",
        ...
        default=None,
        help=(
            "normalizer for proj_cond_on_temb; only effective when "
            "denoiser_model.use_proj_cond_on_temb is enabled in config"
        ),
    )
    parser.add_argument(
...
def main() -> None:
    ...
    if supports_num_verts_scaling:
        ...
        default_num_verts_max = resolve_default_num_verts_max(args)
        pipeline = load_pipeline(
            args,
            num_verts=default_num_verts,
            num_verts_max=default_num_verts_max,
        ...
        default_num_verts_max = read_config_num_verts_max(args.model_path)
